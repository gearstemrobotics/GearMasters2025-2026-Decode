...
public class MeepMeepTesting {
    ...
    public static void main(String[] args) {
        ...
        float isBlue = 1;
        Pose2d beginPose = new Pose2d(new Vector2d(60, isBlue * -12), Math.toRadians(isBlue * -180));
        Pose2d origin = new Pose2d(new Vector2d(0, 0), Math.toRadians(-90));
        Pose2d endPose = new Pose2d(new Vector2d(-45, isBlue * -45), Math.toRadians(isBlue * -135));

        myBot.runAction(myBot.getDrive().actionBuilder(beginPose) //new Pose2d(30, 0, 90))
                // .strafeTo(origin.position)

                .splineToSplineHeading(endPose, isBlue * -134.5)
